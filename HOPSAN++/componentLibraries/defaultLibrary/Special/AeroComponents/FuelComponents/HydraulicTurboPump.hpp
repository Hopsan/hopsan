#ifndef HYDRAULICTURBOPUMP_HPP_INCLUDED
#define HYDRAULICTURBOPUMP_HPP_INCLUDED
...
class HydraulicTurboPump : public ComponentQ
{
private:
     double kl;
     double b;
...
//==This code has been autogenerated using Compgen==
        //Add constants/parameters
            addConstant("kl", "Flow loss koeff.", "", 0.8,kl);
            addConstant("b", "outlet axial width", "m", 0.03,b);
            addConstant("d", "Diameter", "m", 0.3,d);
            addConstant("beta2", "Outlet flow angle", "rad", 1.57,beta2);
            addConstant("Ap", "outlet flow area", "m2", 0.001,Ap);
            addConstant("rho", "Fluid density", "kg/m2", 860,rho);
            addConstant("Kcp", "Leakage coeff", "m3/s/Pa", 1.e-11,Kcp);
...
- 0.159155*q2*rho*Cot(beta2)))/b))/(kl*rho),10.));
          systemEquations[1] =(Power(Ap,2)*(Bp*omegap*(0.01 + 1.*omegap) + \
1.*Kcp*Power(p1,2) - 2.*Kcp*p1*p2 + 1.*Kcp*Power(p2,2) - 1.*p1*q2 + 1.*p2*q2) \
+ kl*(-0.5*Kcp*p1 + 0.5*Kcp*p2 + 0.5*q2)*Power(q2,2)*rho)/(Power(Ap,2)*(-0.01 \
- 1.*omegap)) + torp;
          systemEquations[2] =-c1 + p1 + q2*Zc1;
...
0.5*kl*Power(q2,2)*rho)/(Power(Ap,2)*(-0.01 - 1.*omegap));
          jacobianMatrix[1][1] = 1;
          jacobianMatrix[1][2] = 0. + (Power(Ap,2)*(0. + 2.*Kcp*p1 - \
2.*Kcp*p2 - 1.*q2) + (0. - 0.5*Kcp)*kl*Power(q2,2)*rho)/(Power(Ap,2)*(-0.01 - \
1.*omegap));
          jacobianMatrix[1][3] = 0. + (Power(Ap,2)*(0. - 2.*Kcp*p1 + \
2.*Kcp*p2 + 1.*q2) + (0. + 0.5*Kcp)*kl*Power(q2,2)*rho)/(Power(Ap,2)*(-0.01 - \
1.*omegap));
          jacobianMatrix[2][0] = Zc1;
...
#endif // HYDRAULICTURBOPUMP_HPP_INCLUDED
