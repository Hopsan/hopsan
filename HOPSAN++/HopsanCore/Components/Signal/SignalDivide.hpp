...
public:
    ...
    void simulateOneTimestep()
    {
        ...
        else if (mPorts[in1].isConnected() && !mPorts[in2].isConnected())       //Port 1 connected, port 2 disconnected (no division since no denominator)
        {
            signal1 = p1_ptr->getData(NodeSignal::VALUE);
            signal2 = 1;
        }
        else if (!mPorts[in1].isConnected() && mPorts[in2].isConnected())       //Port 2 connected, port 1 disconnected (nothing to divide, return zero)
        {
            signal1 = 0;
            signal2 = p2_ptr->getData(NodeSignal::VALUE);
        }
