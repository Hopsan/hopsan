...
void GUIPort::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    ...
    if ((!this->isConnected) || (mpParentSystem->mpParentProjectTab->mpSystem->mCoreSystemAccess.getTimeVector(getGUIComponentName(), this->getName()).empty()))
    {
        event->ignore();
    }
    else
...
void GUIPort::plot(QString dataName, QString dataUnit) //En del vansinne i denna metoden...
{
    ...
    QVector<double> time = QVector<double>::fromStdVector(mpParentSystem->mpParentProjectTab->mpSystem->mCoreSystemAccess.getTimeVector(getGUIComponentName(), this->getName()));
    QVector<double> y;
    mpParentSystem->mpParentProjectTab->mpSystem->mCoreSystemAccess.getPlotData(getGUIComponentName(), this->getName(), dataName, y);
...
{
    return mpParentSystem->mpParentProjectTab->mpSystem->mCoreSystemAccess.getPortType(getGUIComponentName(), this->getName());
}
...
{
    return mpParentSystem->mpParentProjectTab->mpSystem->mCoreSystemAccess.getNodeType(getGUIComponentName(), this->getName());
}
