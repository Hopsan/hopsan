...
Special}/HydraulicPistonMload.nb*/
...
//==This code has been autogenerated using Compgen==
        ...
        //Initialize delays
        delayParts1[1] = (2*fmpe*Power(mTimestep,2) - \
2*A1*Power(mTimestep,2)*p1 + 2*A2*Power(mTimestep,2)*p2 + \
2*bL*Power(mTimestep,2)*vmp + 2*bp*Power(mTimestep,2)*vmp - \
8*mL*xmp)/(4.*mL);
        mDelayedPart11.initialize(mNstep,delayParts1[1]);
        delayParts1[2] = (fmpe*Power(mTimestep,2) - A1*Power(mTimestep,2)*p1 \
+ A2*Power(mTimestep,2)*p2 + bL*Power(mTimestep,2)*vmp + \
bp*Power(mTimestep,2)*vmp + 4*mL*xmp)/(4.*mL);
        mDelayedPart12.initialize(mNstep,delayParts1[2]);
        delayParts2[1] = (fmpe*mTimestep - A1*mTimestep*p1 + A2*mTimestep*p2 \
- 2*mL*vmp + bL*mTimestep*vmp + bp*mTimestep*vmp)/(2*mL + bL*mTimestep + \
bp*mTimestep);
    ...
    void simulateOneTimestep()
     {
        ...
        for(iter=1;iter<=mNoiter;iter++)
        {
         ...
         //Differential-algebraic system of equation parts
          ...
          systemEquations[0] =xmp - limit(-(Power(mTimestep,2)*(fmpe - A1*p1 \
+ A2*p2 + bL*vmp + bp*vmp))/(4.*mL) - delayedPart[1][1] - \
delayedPart[1][2],xmin,xmax);
          systemEquations[1] =vmp - dxLimit(limit(-(Power(mTimestep,2)*(fmpe \
- A1*p1 + A2*p2 + bL*vmp + bp*vmp))/(4.*mL) - delayedPart[1][1] - \
delayedPart[1][2],xmin,xmax),xmin,xmax)*(-((mTimestep*(fmpe - A1*p1 + \
A2*p2))/(2*mL + (bL + bp)*mTimestep)) - delayedPart[2][1]);
...
bp)*Power(mTimestep,2)*dxLimit(-(Power(mTimestep,2)*(fmpe - A1*p1 + A2*p2 + \
bL*vmp + bp*vmp))/(4.*mL) - delayedPart[1][1] - \
delayedPart[1][2],xmin,xmax))/(4.*mL);
          jacobianMatrix[0][2] = 0;
          jacobianMatrix[0][3] = 0;
          jacobianMatrix[0][4] = \
-(A1*Power(mTimestep,2)*dxLimit(-(Power(mTimestep,2)*(fmpe - A1*p1 + A2*p2 + \
bL*vmp + bp*vmp))/(4.*mL) - delayedPart[1][1] - \
delayedPart[1][2],xmin,xmax))/(4.*mL);
          jacobianMatrix[0][5] = \
(A2*Power(mTimestep,2)*dxLimit(-(Power(mTimestep,2)*(fmpe - A1*p1 + A2*p2 + \
bL*vmp + bp*vmp))/(4.*mL) - delayedPart[1][1] - \
delayedPart[1][2],xmin,xmax))/(4.*mL);
          ...
          jacobianMatrix[1][4] = \
-((A1*mTimestep*dxLimit(limit(-(Power(mTimestep,2)*(fmpe - A1*p1 + A2*p2 + \
bL*vmp + bp*vmp))/(4.*mL) - delayedPart[1][1] - \
delayedPart[1][2],xmin,xmax),xmin,xmax))/(2*mL + (bL + bp)*mTimestep));
          jacobianMatrix[1][5] = \
(A2*mTimestep*dxLimit(limit(-(Power(mTimestep,2)*(fmpe - A1*p1 + A2*p2 + \
bL*vmp + bp*vmp))/(4.*mL) - delayedPart[1][1] - \
delayedPart[1][2],xmin,xmax),xmin,xmax))/(2*mL + (bL + bp)*mTimestep);
