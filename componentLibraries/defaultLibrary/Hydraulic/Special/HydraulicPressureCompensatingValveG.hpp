...
//! @date Wed 27 Aug 2014 03:23:08
//! @brief A hydraulic pressure compensating valve based on geometry
//! @ingroup HydraulicComponents
//!
//==This code has been autogenerated using Compgen==
//from 
/*{, C:, HopsanTrunk, componentLibraries, defaultLibrary, Hydraulic, \
Special}/HydraulicPressureCompensatingValveG.nb*/
...
//==This code has been autogenerated using Compgen==
        ...
        //Initialize delays
        delayParts1[1] = (Av*mTimestep*p2 - Av*mTimestep*p3 - \
Av*mTimestep*pref - 2*Bv*xv + ks*mTimestep*xv + 2*Cq*mTimestep*w*xv*Abs(p1 - \
p2)*Cos(phi))/(2*Bv + ks*mTimestep + 2*Cq*mTimestep*w*Abs(p1 - p2)*Cos(phi));
    ...
    void simulateOneTimestep()
     {
        ...
        for(iter=1;iter<=mNoiter;iter++)
        {
         ...
         //Differential-algebraic system of equation parts
          ...
          systemEquations[0] =xv - limit(-((Av*mTimestep*(p2 - p3 - \
pref))/(2*Bv + ks*mTimestep + 2*Cq*mTimestep*w*Abs(p1 - p2)*Cos(phi))) - \
...
1.4142135623730951*Cq*Sqrt(1/rho)*w*xv*signedSquareL(p1 - p2,p0);
          ...
          jacobianMatrix[0][2] = (-2*Av*Cq*Power(mTimestep,2)*(p2 - p3 - \
pref)*w*Cos(phi)*dxAbs(p1 - p2)*dxLimit(-((Av*mTimestep*(p2 - p3 - \
pref))/(2*Bv + ks*mTimestep + 2*Cq*mTimestep*w*Abs(p1 - p2)*Cos(phi))) - \
delayedPart[1][1],0.,Xvmax))/Power(2*Bv + ks*mTimestep + \
2*Cq*mTimestep*w*Abs(p1 - p2)*Cos(phi),2);
          jacobianMatrix[0][3] = -((-((Av*mTimestep)/(2*Bv + ks*mTimestep + \
2*Cq*mTimestep*w*Abs(p1 - p2)*Cos(phi))) - (2*Av*Cq*Power(mTimestep,2)*(p2 - \
p3 - pref)*w*Cos(phi)*dxAbs(p1 - p2))/Power(2*Bv + ks*mTimestep + \
2*Cq*mTimestep*w*Abs(p1 - p2)*Cos(phi),2))*dxLimit(-((Av*mTimestep*(p2 - p3 - \
pref))/(2*Bv + ks*mTimestep + 2*Cq*mTimestep*w*Abs(p1 - p2)*Cos(phi))) - \
delayedPart[1][1],0.,Xvmax));
          jacobianMatrix[1][0] = \
