...
1.,&mpFrap);
            addInputVariable("Frat", "Spool circle fraction(A-T)", "", \
1.,&mpFrat);
            addInputVariable("Frbp", "Spool circle fraction(P-B)", "", \
1.,&mpFrbp);
            addInputVariable("Frbt", "Spool circle fraction(B-T)", "", \
1.,&mpFrbt);
            ...
            addInputVariable("thetapa1", "flow angle pa", "rad", \
1.17,&mpthetapa1);
            addInputVariable("thetata1", "flow angle ta", "rad", \
1.17,&mpthetata1);
            addInputVariable("thetapb1", "flow angle pb", "rad", \
1.17,&mpthetapb1);
            addInputVariable("thetatb1", "flow angle tb", "rad", \
1.17,&mpthetatb1);
            addInputVariable("X0f", "flow force distance", "m", \
0.0001,&mpX0f);
            addInputVariable("Xvmax", "Max opening", "m", 0.01,&mpXvmax);
            addInputVariable("plam", "Turbulence onset pressure", "Pa", \
10000.,&mpplam);
        //Add outputVariables to the component
            addOutputVariable("ffpa","flow force pa","N",0.,&mpffpa);
            addOutputVariable("ffta","flow force ta","N",0.,&mpffta);
            addOutputVariable("ffpb","flow force pb","N",0.,&mpffpb);
            addOutputVariable("fftb","flow force tb","N",0.,&mpfftb);
            addOutputVariable("ff","Total flow force","N",0.,&mpff);
...
//==This code has been autogenerated using Compgen==
          ...
          pb=stateVark[7];
          //Expressions
          ffpa = -6.28319*Cq*Frap*(-pa + pp)*Sd*cos((1 - \
Power(2.71828,-limit(Xap0 + xv,0.,Xap0 + Xvmax)/X0f))*thetapa1)*limit(Xap0 + \
xv,0.,Xap0 + Xvmax);
          ffta = -6.28319*Cq*Frat*(-pa + pt)*Sd*cos((1 - \
Power(2.71828,-limit(Xap0 - xv,0.,Xat0 + Xvmax)/X0f))*thetata1)*limit(Xap0 - \
xv,0.,Xat0 + Xvmax);
          ffpb = 6.28319*Cq*Frbp*(-pb + pp)*Sd*cos((1 - \
Power(2.71828,-limit(Xap0 - xv,0.,Xbp0 + Xvmax)/X0f))*thetapb1)*limit(Xap0 - \
xv,0.,Xbp0 + Xvmax);
          fftb = 6.28319*Cq*Frbt*(-pb + pt)*Sd*cos((1 - \
Power(2.71828,-limit(Xap0 + xv,0.,Xbt0 + Xvmax)/X0f))*thetatb1)*limit(Xap0 + \
xv,0.,Xbt0 + Xvmax);
          ff = ffpa + ffpb + ffta + fftb;
        }
