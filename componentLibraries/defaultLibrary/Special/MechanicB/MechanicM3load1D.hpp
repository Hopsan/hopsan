...
//==This code has been autogenerated using Compgen==
        ...
        //Add inputVariables to the component
            addInputVariable("fc1","Dry friction 1 (+/-)","N",10.,&mpfc1);
            addInputVariable("fc2","Dry friction 2 (+/-)","N",10.,&mpfc2);

        //Add inputParammeters to the component
            addInputVariable("m1", "Inertia1", "kg", 30.,&mpm1);
            addInputVariable("m2", "Inertia2", "kg", 30.,&mpm2);
            addInputVariable("m3", "Inertia2", "kg", 1000.,&mpm3);
            addInputVariable("bL", "Visc. friction coeff.", "Ns/m", \
10.,&mpbL);
            addInputVariable("bfc", "Numerical friction factor.", "", \
1.,&mpbfc);
            addInputVariable("xpmin", "Limitation on stroke x1,x2", "m", \
0.,&mpxpmin);
            addInputVariable("xpmax", "Limitation on stroke x1,x2", "m", \
1.,&mpxpmax);
        //Add outputVariables to the component
            addOutputVariable("xp1","piston 3 position","m",0.,&mpxp1);
            addOutputVariable("xp2","piston 2 position","m",0.,&mpxp2);
            addOutputVariable("xL","Load position","m",0.,&mpxL);
            addOutputVariable("fc13","friction force 1-3","m",0.,&mpfc13);
            addOutputVariable("fc23","friction force 2-3","m",0.,&mpfc23);
...
//==This code has been autogenerated using Compgen==
        ...
        //Initialize delays
        delayParts1[1] = (fm1*mTimestep - 2*m1*vm1 - \
mTimestep*limit((-2*bfc*m1*m3*(vm1 + vm3))/((m1 + \
m3)*mTimestep),-fc1,fc1))/(2.*m1);
        mDelayedPart11.initialize(mNstep,delayParts1[1]);
        delayParts2[1] = (fm2*mTimestep - 2*m2*vm2 - \
mTimestep*limit((-2*bfc*m2*m3*(vm2 + vm3))/((m2 + \
m3)*mTimestep),-fc2,fc2))/(2.*m2);
        mDelayedPart21.initialize(mNstep,delayParts2[1]);
        delayParts3[1] = (fm3*mTimestep - 2*m3*vm3 - \
mTimestep*limit((-2*bfc*m1*m3*(vm1 + vm3))/((m1 + m3)*mTimestep),-fc1,fc1) - \
mTimestep*limit((-2*bfc*m2*m3*(vm2 + vm3))/((m2 + \
m3)*mTimestep),-fc2,fc2))/(2.*m3);
...
delayedPart[4][1],-xpmax,-xpmin),-xpmax,-xpmin)*(-(mTimestep*(fm1 - \
limit((-2*bfc*m1*m3*(vm1 + vm3))/((m1 + m3)*mTimestep),-fc1,fc1)))/(2.*m1) - \
delayedPart[1][1]);
          systemEquations[1] =vm2 - dxLimit(limit((mTimestep*vm2)/2. - \
delayedPart[5][1],-xpmax,-xpmin),-xpmax,-xpmin)*(-(mTimestep*(fm2 - \
limit((-2*bfc*m2*m3*(vm2 + vm3))/((m2 + m3)*mTimestep),-fc2,fc2)))/(2.*m2) - \
delayedPart[2][1]);
          systemEquations[2] =vm3 + (mTimestep*(fm3 - \
limit((-2*bfc*m1*m3*(vm1 + vm3))/((m1 + m3)*mTimestep),-fc1,fc1) - \
limit((-2*bfc*m2*m3*(vm2 + vm3))/((m2 + m3)*mTimestep),-fc2,fc2)))/(2.*m3) + \
delayedPart[3][1];
          systemEquations[3] =xm1 - limit((mTimestep*vm1)/2. - \
delayedPart[4][1],-xpmax,-xpmin);
          systemEquations[4] =xm2 - limit((mTimestep*vm2)/2. - \
delayedPart[5][1],-xpmax,-xpmin);
          systemEquations[5] =-(mTimestep*vm3)/2. + xm3 + delayedPart[6][1];
          systemEquations[6] =-cm1 + fm1 + vm2*Zcm1;
          systemEquations[7] =-cm2 + fm2 - vm2*Zcm2;
          systemEquations[8] =-cm3 + fm3 - vm3*Zcm3;

          //Jacobian matrix
          jacobianMatrix[0][0] = 1 + (bfc*m3*dxLimit((-2*bfc*m1*m3*(vm1 + \
vm3))/((m1 + m3)*mTimestep),-fc1,fc1)*dxLimit(limit((mTimestep*vm1)/2. - \
delayedPart[4][1],-xpmax,-xpmin),-xpmax,-xpmin))/(m1 + m3);
          jacobianMatrix[0][1] = 0;
          jacobianMatrix[0][2] = (bfc*m3*dxLimit((-2*bfc*m1*m3*(vm1 + \
vm3))/((m1 + m3)*mTimestep),-fc1,fc1)*dxLimit(limit((mTimestep*vm1)/2. - \
delayedPart[4][1],-xpmax,-xpmin),-xpmax,-xpmin))/(m1 + m3);
...
- delayedPart[4][1],-xpmax,-xpmin),-xpmax,-xpmin))/(2.*m1);
          jacobianMatrix[0][7] = 0;
          jacobianMatrix[0][8] = 0;
          jacobianMatrix[1][0] = 0;
          jacobianMatrix[1][1] = 1 + (bfc*m3*dxLimit((-2*bfc*m2*m3*(vm2 + \
vm3))/((m2 + m3)*mTimestep),-fc2,fc2)*dxLimit(limit((mTimestep*vm2)/2. - \
delayedPart[5][1],-xpmax,-xpmin),-xpmax,-xpmin))/(m2 + m3);
          jacobianMatrix[1][2] = (bfc*m3*dxLimit((-2*bfc*m2*m3*(vm2 + \
vm3))/((m2 + m3)*mTimestep),-fc2,fc2)*dxLimit(limit((mTimestep*vm2)/2. - \
delayedPart[5][1],-xpmax,-xpmin),-xpmax,-xpmin))/(m2 + m3);
...
- delayedPart[5][1],-xpmax,-xpmin),-xpmax,-xpmin))/(2.*m2);
          jacobianMatrix[1][8] = 0;
          jacobianMatrix[2][0] = (bfc*m1*dxLimit((-2*bfc*m1*m3*(vm1 + \
vm3))/((m1 + m3)*mTimestep),-fc1,fc1))/(m1 + m3);
          jacobianMatrix[2][1] = (bfc*m2*dxLimit((-2*bfc*m2*m3*(vm2 + \
vm3))/((m2 + m3)*mTimestep),-fc2,fc2))/(m2 + m3);
          jacobianMatrix[2][2] = 1 + \
(mTimestep*((2*bfc*m1*m3*dxLimit((-2*bfc*m1*m3*(vm1 + vm3))/((m1 + \
m3)*mTimestep),-fc1,fc1))/((m1 + m3)*mTimestep) + \
(2*bfc*m2*m3*dxLimit((-2*bfc*m2*m3*(vm2 + vm3))/((m2 + \
m3)*mTimestep),-fc2,fc2))/((m2 + m3)*mTimestep)))/(2.*m3);
          jacobianMatrix[2][3] = 0;
          jacobianMatrix[2][4] = 0;
          jacobianMatrix[2][5] = 0;
          jacobianMatrix[2][6] = 0;
          jacobianMatrix[2][7] = 0;
          jacobianMatrix[2][8] = mTimestep/(2.*m3);
          jacobianMatrix[3][0] = -(mTimestep*dxLimit((mTimestep*vm1)/2. - \
delayedPart[4][1],-xpmax,-xpmin))/2.;
...
delayedPart[5][1],-xpmax,-xpmin))/2.;
          ...
          jacobianMatrix[5][2] = -mTimestep/2.;
...
//==This code has been autogenerated using Compgen==
          ...
          xp2 = -xm2;
          xL = xm3;
          fc13 = limit((-2*bfc*m1*m3*(vm1 + vm3))/((m1 + \
m3)*mTimestep),-fc1,fc1);
          fc23 = limit((-2*bfc*m2*m3*(vm2 + vm3))/((m2 + \
m3)*mTimestep),-fc2,fc2);
          eqMassm1 = m1;
          eqMassm2 = m2;
          eqMassm3 = m3;
        }
