...
AeroComponents}/AeroAircraft6DOFSSg.nb*/
...
ceff","",0.,&mpdeCY);
            ...
            addInputVariable("thrustl","Left ngine \
thrust","N",0.,&mpthrustl);
            addInputVariable("thrustr","Right ngine \
...
- Liftr2 - deCL*qpress*S1)*Sin(Alpha) - Cos(Alpha)*Sin(Beta)*(-Cfin - \
deCY*qpress*S1 - Cydeelev*qpress*S2*(thetaal2 - thetaar2) + \
CLalphabv*qpress*Sbv*Sin(Beta));
        Fy = (-Dragb - Dragfin - Dragl1 - Dragl2 - Dragr1 - Dragr2 - \
deCD*qpress*S1)*Sin(Beta) + Cos(Beta)*(-Cfin - deCY*qpress*S1 - \
...
deCL*qpress*S1)*Cos(Alpha) - kground*zcg*onPositive(zcg) + (-Dragb - Dragfin \
- Dragl1 - Dragl2 - Dragr1 - Dragr2 - deCD*qpress*S1)*Cos(Beta)*Sin(Alpha) - \
Sin(Alpha)*Sin(Beta)*(-Cfin - deCY*qpress*S1 - Cydeelev*qpress*S2*(thetaal2 - \
thetaar2) + CLalphabv*qpress*Sbv*Sin(Beta)) - \
thrustl*Cos(deythrustl)*Sin(dezthrustr) - \
thrustr*Cos(deythrustr)*Sin(dezthrustr);
        Lb = (Liftl1 - Liftr1)*rc1 + (Liftl2 - Liftr2)*rc2 + deCl*qpress*S1 - \
...
thrustr*Cos(deythrustr)*Sin(dezthrustr));
        Mb = deCm*qpress*S1 - Mdvtheta*(Qb + 2*(q1*q2 + q0*q3)*wturbx + \
(Power(q0,2) - Power(q1,2) + Power(q2,2) - Power(q3,2))*wturby + 2*(-(q0*q1) \
...
+ thrustr*Cos(deythrustr)*Sin(dezthrustr));
        Nb = (-Dragl1 + Dragr1)*rc1 + (-Dragl2 + Dragr2)*rc2 + deCn*qpress*S1 \
- Mdvpsi*(Rb + 2*(-(q0*q2) + q1*q3)*wturbx + 2*(q0*q1 + q2*q3)*wturby + \
(Power(q0,2) - Power(q1,2) - Power(q2,2) + Power(q3,2))*wturbz) - Cfin*(-xbcg \
+ xwfin) + (xbcg - xeng)*(thrustl*Cos(dezthrustl)*Sin(deythrustl) + \
thrustr*Cos(dezthrustr)*Sin(deythrustr));
...
Power(q2,2) - Power(q3,2));
          ...
          gfy = Fy/mass;
          gfz = Fz/mass;
          Faz = -Liftb - Liftl1 - Liftl2 - Liftr1 - Liftr2 - deCL*qpress*S1;
