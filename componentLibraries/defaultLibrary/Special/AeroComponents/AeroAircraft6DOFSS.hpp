...
AeroComponents}/AeroAircraft6DOFSS1.nb*/
...
(Power(epsM,2)*Power(v,2))/Power(vM,2),0.25);
        ...
        e1e = e1 - ((1 - 1/AR1)*e1)/(1 + Power(en,(-2*(v - vM))/(dM*vM)));
        e2e = e2 - ((1 - 1/AR2)*e2)/(1 + Power(en,(-2*(v - vM))/(dM*vM)));
...
q1*q3)*wturbz))/(0.1 + v) + rc1*rc2*((Liftl1 - Liftr1)*Cos(Alpha) - (Dragl1 - \
Dragr1)*Sin(Alpha)) + rc2*((Liftl2 - Liftr2)*Cos(Alpha) - (Dragl2 - \
Dragr2)*Sin(Alpha)) + yeng*(thrustl*Cos(deythrustl)*Sin(dezthrustl) - \
thrustr*Cos(deythrustr)*Sin(dezthrustr));
...
Power(en,(-2*(v - vM))/(dM*vM)))) - (smc*(CLde1*thetaar1 + CLde12*thetaar12 + \
CLift(Alpha*dah1 - ia1,CLalpha1e,ap1,an1,awp1,awn1)/2.))/(4.*(1 + \
Power(en,(-2*(v - vM))/(dM*vM)))) + CMoment(Alpha*dah1 - \
ia1,Cm01,Cmfs1,ap1,an1,awp1,awn1)) + (xbcg - xw1)*((Liftl1 + \
Liftr1)*Cos(Alpha) + (Dragl1 + Dragr1)*Sin(Alpha)) + (xbcg - xw2)*(-Cl2 - Cr2 \
+ (Liftl2 + Liftr2)*Cos(Alpha) - (Dragl2 + Dragr2)*Sin(Alpha)) + (xbcg - \
xeng)*(thrustl*Cos(deythrustl)*Sin(dezthrustl) + \
thrustr*Cos(deythrustr)*Sin(dezthrustr));
        Nb = -(Mdvpsi*(Rb + 2*(-(q0*q2) + q1*q3)*wturbx + 2*(q0*q1 + \
q2*q3)*wturby + (Power(q0,2) - Power(q1,2) - Power(q2,2) + \
Power(q3,2))*wturbz)) - Cfin*(-xbcg + xwfin) + rc1*((-Dragl1 + \
