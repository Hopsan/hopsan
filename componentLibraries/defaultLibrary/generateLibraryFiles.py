...
def generatePRI(dirPath, filename, includeFiles, componentHeaders, componentSrcFiles, componentCCIFiles):
    ...
    if not file.closed:
        ...
        file.write("#This file has been automatically generated by generateLibraryFiles.py!\n")
        for includeFile in includeFiles:
            file.write(r'include($${PWD}/'+os.path.relpath(includeFile.replace('\\', '/'), dirPath)+r')'+"\n")
        file.write("\n")
        file.write("\n")

        file.write(r'HEADERS += ')
        for header in componentHeaders:
            file.write(r' \ '+'\n'+r' $${PWD}/'+os.path.relpath(header.replace('\\', '/'), dirPath))
        file.write("\n")
        file.write("\n")

        file.write(r'SOURCES += ')
        for source in componentSrcFiles:
            file.write(r' \ '+"\n"+r' $${PWD}/'+os.path.relpath(source.replace('\\', '/'), dirPath))
        file.write("\n")
        file.write("\n")

        file.write(r'OTHER_FILES += ')
        for other in componentCCIFiles:
            file.write(r' \ '+"\n"+r' $${PWD}/'+os.path.relpath(other.replace('\\', '/'), dirPath))
        file.write("\n")
        file.write("\n")
    file.close()
    return filepath
