    ...
    public:
        ...
        void initialize()
        {
            if ( mLookupTable.isEmpty() || mReloadCSV )
            {
                ...
                isOK = mDataCurve.setFile(findFilePath(mDataCurveFileName));
                if (isOK)
                {
                    isOK = mDataCurve.parseWholeFile();
                }
                if(!isOK)
                {
                    addErrorMessage("Unable to initialize CSV file: "+mDataCurveFileName+", "+mDataCurve->getErrorString());
                    stopSimulation();
                }
                else
                {
                    // Make sure that selected data vector is in range
                    const int nDataCols = mDataCurve->getNumDataCols();
                    ...
                    mDataCurve.copyColumn(mInDataId, mLookupTable.getIndexDataRef());
                    mDataCurve.copyColumn(mOutDataId, mLookupTable.getValueDataRef());
                    // Now the data is in the lookuptable and we can throw away the csv data to conservev memory
                    mDataCurve.clearData();
                    ...
                    if(!isOK)
                    {
                        ...
                        if (mLookupTable.isIndexIncreasingOrDecresing(0) != LookupTableND<1>::StrictlyIncreasing)
                        {
