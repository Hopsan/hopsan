...
mkdir %builddir_d%
pushd %builddir_d%
cmake -Wno-dev -G %HOPSAN_BUILD_CMAKE_GENERATOR% -DCMAKE_BUILD_TYPE=Debug -DCMAKE_DEBUG_POSTFIX="d" -DCMAKE_INSTALL_PREFIX=%installdir% -DFMI4C_BUILD_SHARED=OFF -DFMI4C_USE_INCLUDED_ZLIB=ON %codedir%
...
mkdir %builddir_r%
pushd %builddir_r%
cmake -Wno-dev -G %HOPSAN_BUILD_CMAKE_GENERATOR% -DCMAKE_BUILD_TYPE=Release -DCMAKE_DEBUG_POSTFIX="d" -DCMAKE_INSTALL_PREFIX=%installdir% -DFMI4C_BUILD_SHARED=OFF -DFMI4C_USE_INCLUDED_ZLIB=ON %codedir%
